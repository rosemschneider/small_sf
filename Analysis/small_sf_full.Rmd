...
subset.chance.within.kl <- glmer(Correct ~ 1 + (1|SID), family = "binomial", 
                                 data = subset(model.df.study1, within_kl == "Within"))
...
subset.hc.within <- glmer(Correct ~ highest_count + (1|SID), 
                          family = "binomial", data = subset(model.df.study1, within_kl == "Within"))
...
#make base
cp.subset.base. <- glmer(Correct ~ age.c + (1|SID), 
                      family = 'binomial', data = subset(model.df.study2, Task == "SF"))
#add cp_subset
cp.subset.kl <- glmer(Correct ~ CP_subset + age.c + (1|SID), 
                      family = 'binomial', data = subset(model.df.study2, Task == "SF"))
#compare
anova(cp.subset.base, cp.subset.kl, test = 'LRT')
#add highest_count
cp.subset.hc <- glmer(Correct ~ highest_count + age.c + (1|SID), 
                      family = 'binomial', data = subset(model.df.study2, Task == "SF"))
