...
While @sarnecka2008 found that CP-knowers outperformed subset knowers on the Unit Task, there is strong evidence indicating that their Unit Task success did not show evidence of implicit knowledge of the successor function at all, but instead likely reflected item-based knowledge with specific numbers. First, not all CP-knowers in @sarnecka2008 were able to pass the Unit Task for 4 and 5 (numbers well within their known number range), such that the group overall only achieved 67% accuracy. Second, subsequent work has found that knowledge of the count list, rather than CP-knowledge in general, is a stronger predictor children's ability to implement the successor function for \emph{any} number, and not simply those within their count list. For example, @davidson2012 found that CP-knowers who were able to count higher without error could implement the successor function for all numbers in their count list, while CP-knowers who were less proficient counters were at chance on the Unit Task even for numbers well within their counting range. @cheung2017 subsequently replicated and extended this finding, showing that CP-knowers' were not uniformly successful for a large range of numbers on the Unit Task until they could count quite high (>80) without error. Most recently, @schneider2020 found that this more generalized successor knowledge was strongly predicted by children's knowledge of the productive nature of the count list. 

Taken together, these results suggest that CP-knowers' scattered Unit Task success does not necessarily indicate knowledge of the successor function. Instead, it seems that young CP-knowers' ability to reason about the set operations entailed in the Unit Task for a subset of numbers in their count list may instead reflect their familiarity with specific numbers, rather than an induction made over the count list. @davidson2012 proposed that this item-specific knowledge may lay the groundwork for a greater induction about the successor function; that is, learning that the result of adding 1 to a given \emph{N} should be labeled by the next number in the count list, \emph{N}+1 for some numbers may provide children with the beginnings of a generalized mechanism that they can later expand to include all numbers in the count list. Thus, while children's limited Unit Task success may not be evidence of implicit successor knowledge, it may reflect the origins of a localized network of item-based network that may eventually form the basis of children's discovery that successive numbers in the count list all differ by a cardinality of exactly 1. 

How do children begin to establish these item-based mappings? One possibility is that access to the structure of the count list is a necessary precondition for any kind of successor knowledge, however limited. On this hypothesis, only CP-knowers should be able to reason about item-based successor relations, as they are supported by mapping the relationship between set operations and known portions of the count list [@spaepen2018], and that their ability to do so is constrained by their count list mastery. Supporting this view, CP-knowers routinely outperform subset knowers on the Unit Task for numbers within their rote counting range [@davidson2012], and even for sets of 1 and 2 [@sarnecka2008]. Additionally, @spaepen2018 found that CP-knowers' Unit Task performance on sets of 5 and 6 improved with counting training, while subset knowers' performance did not. Thus, it is possible that while the CP is not sufficient to impart generalized successor knowledge, children must be able to make an analogical mapping between the next number in the count list and cardinality to begin acquiring the general mechanism supporting the successor function. 

While subset knowers' failures on the Unit Task are consistent with the above analaogical mapping account, an alternative explanation may be due to the fact that subset knowers are typically tested on numbers which fall well outside their known number range, such as 4, 5, and 6. Because children's item-specific Unit Task success seems more closely related to their familiarity with specific numbers, subset knowers should necessarily fail on these "large" numbers. In fact, 
despite lower performance in comparison to CP-knowers, many subset knowers demonstrate above-chance Unit Task performance for sets of 1 and 2, numbers well within their known number range [@sarnecka2008]. Additionally, there is some evidence that subset knowers are able to perform the basic set operations in working memory required to succeed on the Unit Task, even without having access to the count list's ordinal structure. Beginning at five months, infants are able to track the addition of 1 item to an occluded set of 1, demonstrating surprise if the occluded set reveals 1, but not 2, items [@wynn1992addition]. @huttenlocher1994 found that children between 2.5 and 4 years were able to track additions of 1 to sets of 1, 2, and 3. Finally, @hughes1981 found that children as young as 3 years were able to succeed on a paradigm similar to the Unit Task for small numbers (1, 2, and 3). Taken together, these findings raise the possibility that children may begin learning these item-based mappings prior to acquiring the CP, but that previous work has not been able to detect this early knowledge because it queries numbers which are unknown to subset knowers.
