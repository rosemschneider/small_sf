...
# Introduction
In the first few years of life, children accumulate knowledge of the components of symbolic number in isolation, gradually combining and integrating them to arrive at increasingly abstract numerical concepts. Throughout this process, however, their understanding of number is often strikingly limited, indicating fragile or absent mappings between components. A key example of this phenomenon is in children's protracted discovery that adding 1 to a set always denotes a number word 1 greater in the count list -- an understanding that reflects some implicit knowledge of the \emph{successor function}, a logical principle stating that the successor of any number \emph{n} is \emph{n}+1. For 1.5 -- 2 years after children have learned how to use the count routine to generate sets, they demonstrate piecemeal successor knowledge, and can implement it for only some numbers in their count list [@davidson2012; @cheung2017]. While there is growing evidence that children eventually use their count list knowledge to generalize the successor function to \emph{all} numbers, there has been comparatively little work exploring their early item-based understanding. In the current work, we investigate the origins of children's successor knowledge by exploring the mechanisms through which they begin to acquire these localized mappings.

Children's generalization of the successor function comes several years after an extended process of acquiring number word meanings and the count routine. While most US children can recite portions of the count list around 2 years of age [@fuson1988], it is not until around 2.5 years that they begin to understand the meanings of some number words [@wynn1990]. These "subset knowers" learn the first few number words (generally \emph{one}, \emph{two}, and \emph{three}) sequentially and in isolation over a period of several months, but do not recognize the connection between the cardinalities represented by these number words and the count list. For example, a "two-knower" might be able to generate sets of \emph{one} and \emph{two}, while having no knowledge of \emph{three} [@wynn1990; @wynn1992]. Around 4 years of age, however, children eventually make this connection, and realize how the last word said while counting indicates the cardinality of a set (i.e. the Cardinal Principle, or CP, @gelman1978).

On some accounts, the process underlying CP acquisition entails noticing that the cardinalities associated with known number words all differ by exactly 1, and that this is reflected by a +1 difference in the count list [@carey2004; @sarnecka2008]. Some evidence for this comes from a paradigm knwon as the "Unit Task" [@sarnecka2008]. In this task, an experimenter labels a set ("Look! Here are 4 frogs!") and hides them behind an occluder. The experimenter then adds 1 item to this set and asks, "Are there 5 or 6 frogs now?". To succeed, children must make an analogical mapping; that is, they must understand that the addition of 1 item to an established cardinality necessitates moving up exactly 1 number in the count list. Compatible with the hypothesis that acquiring the CP entails implicit knowledge of the successor function, @sarnecka2008 found that CP-knowers were signficantly above chance on the Unit Task for sets of 4 and 5, while subset knowers performed at chance. 

While CP-knowers in Sarnecka and Carey (2008) outperformed subset knowers, there is strong evidence indicating that their Unit Task success did indicate generalized implicit succesor knowledge at all, but instead likely reflected item-based knowledge of specific numbers. First, while CP-knowers in Sarnecka and Carey (2008) performed above chance (50%), their average performance for even 4 and 5 was only 67%. Second, subsequent work has found that count list mastery is a stronger predictor than CP knowledge of Unit Task performance. For example, @davidson2012 found that CP-knowers who could count higher without error passed the Unit Task for all numbers in their count list, while less proficient counters were at chance for even well-known numbers. @cheung2017 subsequently replicated and extended this finding, showing that CP-knowers were not uniformly successful for larger numbers on the Unit Task until they could count quite high (>80) without error. Most recently, @schneider2020 found that this more generalized successor knowledge was strongly predicted by children's knowledge of the productive nature of the count list. 

Taken together, these results suggest that CP-knowers' scattered Unit Task success does not necessarily indicate a generalized knowledge of the successor function. Instead, it seems that young CP-knowers' ability to reason about the set operations entailed in the Unit Task for a subset of numbers in their count list may instead item-based familiarity with specific numbers -- e.g., that \emph{six} is 1 more than \emph{five}, rather than a more general principle that, for \emph{any} number, moving up in the count list corresponds to adding +1. @davidson2012 proposed that this item-specific knowledge may lay the groundwork for a greater induction about the successor function; that is, learning that the result of adding 1 to a given \emph{N} should be labeled by the next number in the count list, \emph{N}+1 for some numbers may provide children with the beginnings of a generalized mechanism that they can later expand to include all numbers in the count list. Thus, while children's limited Unit Task success may not be evidence of implicit successor knowledge, it may reflect the origins of a localized network which eventually forms the basis of children's discovery that successive numbers in the count list all differ by a cardinality of exactly 1. 

Although multiple studies have asked how children acquire this generalized function, there has been little work done on how children begin to establish these item-based mappings. One possibility is that they are supported by knowledge of the count list, and how changes in cardinality are reflected in relations between number words. This account, similar to Carey's (2004) proposal, predicts that only CP-knowers should be able to demonstrate this item-based knowledge, as subset knowers have not yet recognized the count list's relationship to cardinality. Some support for this account comes from @spaepen2018, who found that CP-knowers who were trained on a portion of the count list containing \emph{five}, \emph{six}, and \emph{seven} had greater Unit Task performance in comparison to children who did not have this training. Notably, subset knowers did not improve with counting training, suggesting that CP knowers may have been using count list knowledge to pass the Unit Task. Additionally, there is robust evidence that CP-knowers routinely outperform subset knowers on the Unit Task [@davidson2012; @sarnecka2008]. Thus, it is possible that while the CP is not sufficient to impart generalized successor knowledge, children may only be in a position to acquire item-based mappings through recognizing an isomorphism between set operations and relationships between known number words.

An alternative to this analogical mapping account, however, is that children may begin establishing this item-based knowledge before recognizing the isomorphism between the count list and cardinality by mapping the results of set operations directly to known number words \emph{without} reasoning about relations between these number words. That is, a child who knows that \emph{three} corresponds to a set of three items could succeed on the Unit Task by tracking the addition of 1 to a set of 2, and using their set-based mapping between a known number word and a nonsymbolic representation of the final set. On this account, item-based knowledge is not dependent upon acquisition of the CP, as children are mapping directly from nonsymbolic representations of sets to known number words, rather than using knowledge of relations between number words in the count list to reason about set operations. One reason that previous work has found that CP-knowers typically outperform subset knowers on the Unit Task may be that children are typically tested on numbers which fall well outside subset knowers' known number range, such as 4, 5, and 6; on this account, subset knowers should necessarily fail on these "large" numbers. 
In fact, despite lower performance in comparison to CP-knowers, many subset knowers demonstrate above-chance Unit Task performance for sets of 1 and 2, numbers well within their known number range [@sarnecka2008]. Additionally, there is some evidence that subset knowers are able to perform the basic set operations in working memory required to succeed on the Unit Task, even without having access to the count list's ordinal structure. @huttenlocher1994 found that children between 2.5 and 4 years were able to track additions of 1 to sets of 1, 2, and 3, while @hughes1981 found that children as young as 3 years were able to succeed on a paradigm similar to the Unit Task for small numbers (1, 2, and 3).

Taken together, these findings raise the possibility that children may begin learning these item-based mappings prior to acquiring the CP, but that previous work has not been able to detect this early knowledge because it queries numbers which are unknown to subset knowers.

In the current work, we explore these two hypotheses by investigating subset and CP-knowers’ knowledge of item-based mappings for very small sets (1 - 5). In Experiment 1, we show that subset knowers’ Unit Task performance was significantly above chance for familiar items within their knower-level, indicating that children begin to establish this item-based network even before acquiring the CP. In Experiment 2 we explored the two possible mechanisms underlying this early Unit Task success, and find no evidence that subset knowers are drawing upon the count list to form these item-based mappings. 

<!-- many CP-knowers still do not understand how its ordinal structure captures the successor function. For example, in a paradigm frequently used to assess successor function knowledge known as the "Unit Task," many young CP-knowers are unable to determine whether adding +1 to sets of 4, 5, or 6 (numbers well within their count range) results in a set which should be labeled by \emph{n}+1 or \emph{n}+2 [@sarnecka2008;@spaepen2018]. This limited knowledge extends for at least one to two years after learning the CP, suggesting that children's earliest successor mappings may be drawn from their familiarity with specific numbers, as opposed to a generalized principle. There is growing evidence, however, that children may eventually learn how the successor function can be implemented for all numbers through their mastery of the count list.  counting proficiency and Unit Task performance were significantly correlated with their understanding of numerical infinity.  -->
# Experiment 1
## Method
...
#### Highest Count. 
We used the Highest Count task as a measure of counting experience beyond the CP stage, as well as to test whether, even in the subset stages, knowledge of counting procedures might enhance knowledge of relations between words. An experimenter introduced the task to the child by saying, “In this game, I want you to count as high as you can! Can you start counting for me with \emph{one}?" 

A child’s Highest Count was defined as the largest number counted to before making an error, or the point at which the child did not know how to continue. The first time a child stopped counting the experimenter prompted them, saying “Do you know what comes next?” If a child could not continue, the task was ended. Otherwise, the child was able to continue counting until they stopped again. 
